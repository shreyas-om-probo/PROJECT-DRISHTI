...
class CustomHTTPException(HTTPException):
    def __init__(self, status_code: int, detail: str, internal_error: str = None):
        ...
        logger.info(f"HTTP Exception raised - Status: {status_code}, Detail: {detail}, Internal Error: {internal_error}")
...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        logger.info(f"Request failed - ID: {request_id} - Error: {str(e)}\n{traceback.format_exc()}")
        raise
...
if not PERPLEXITY_API_KEY:
    logger.info("PERPLEXITY_API_KEY not found in environment variables")

class TextInput(BaseModel):
    text: str

def extract_json_for_against(response: str) -> list:
    logger.info(f"Attempting to extract JSON from response of length: {len(response)}")
    try:
        ...
        logger.info(f"Found {len(for_against_data)} items with For/Against structure")
        ...
        logger.info(f"Unexpected error in extract_json_for_against: {str(e)}\n{traceback.format_exc()}")
        raise CustomHTTPException(
            status_code=500,
...
async def event_generate_v1(user_prompt: TextInput) -> Dict[str, Any]:
    ...
    try:
        logger.info(f"Request ID {request_id} - Calling LLM overview")
        overview = llm_overview.call_llm(user_prompt, sot=[], recency="year")
        
        logger.info(f"Request ID {request_id} - Calling LLM combined")
        combined = llm_combined.call_llm(user_prompt, sot=[], recency="year")
        
        logger.info(f"Request ID {request_id} - Extracting For/Against data")
        try:
            arguments = extract_json_for_against(combined[0])
        except:
                logger.info("Initial json extraction failed trying again.")
                logger.info(f"Request ID {request_id} - Calling LLM combined again")
                combined = llm_combined.call_llm(user_prompt, sot=[], recency="year")
                
                logger.info(f"Request ID {request_id} - Extracting For/Against data again")
                arguments = extract_json_for_against(combined[0])
        ...
        logger.info(f"Request ID {request_id} - {error_msg}\n{traceback.format_exc()}")
        raise CustomHTTPException(
            status_code=500,
            detail=error_msg,
...
def search_news(request: TextInput):
    ...
    try:
        ...
        logger.info(f"Request ID {request_id} - {error_msg}: {str(e)}\n{traceback.format_exc()}")
        raise CustomHTTPException(
            status_code=500,
            detail=error_msg,
            internal_error=traceback.format_exc()
...
async def post_index(payload: TextInput):
    request_id = os.urandom(6).hex()
    logger.info(f"---------------------------------------- INSIGHTS API Call Made at {datetime.now()}----------------------------------------")
    logger.info(f"Request ID {request_id} - Processing combined insights request: {payload.text[:100]}...")
    
    async with httpx.AsyncClient() as client:
        try:
            logger.info(f"Request ID {request_id} - Fetching tweets")
            tweets_response = await client.post(
                "http://127.0.0.1:8007/search-tweets",
            ...
            logger.info(f"Request ID {request_id} - Fetching insights")
            analyze_response = await client.post(
                "http://127.0.0.1:8007/insights",
                json={"text": payload.text},
                timeout=30.0
            )
            ...
            logger.info(f"Request ID {request_id} - Fetching news")
            news_response = await client.post(
                "http://127.0.0.1:8007/search-news",
                json={"text": payload.text},
                timeout=30.0
            )
...
async def post_index(request: Request, text: str = Form(...)):
    logger.info(f"---------------------------------------- UI BASED INSIGHTS Call Made at {datetime.now()}----------------------------------------")
    request_id = os.urandom(6).hex()
...
if __name__ == '__main__':
    ...
    logger.info(f"Server URL: http://127.0.0.1:8007")
    logger.info(f"Environment: {os.getenv('ENVIRONMENT', 'development')}")
    logger.info("="*50)
